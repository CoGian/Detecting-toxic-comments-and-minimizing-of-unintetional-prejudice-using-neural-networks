...
def encode_examples(df, PATH, sample_weights=None, labels=None, labels_aux=None, forTest=False):
  roberta_input = tokenizer.batch_encode_plus(df['comment_text'].astype(str).values,
  ...
  with open(PATH + '/input_ids.npy', 'wb') as filehandle:
    ...
    np.save(filehandle, np.asarray(roberta_input['input_ids']))
  with open(PATH + '/input_mask.npy', 'wb') as filehandle:
    # store the data as binary data stream
    np.save(filehandle, np.asarray(roberta_input['attention_mask']))
...
encode_examples(train_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/RoBERTa/data/train', sample_weights_train, y_train, y_train_aux)
print("Finished train data..")
encode_examples(val_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/RoBERTa/data/val', sample_weights_val, y_val, y_val_aux)
print("Finished val data..")
encode_examples(test_private_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/RoBERTa/data/test_private', forTest=True)
print("Finished test private data..")
encode_examples(test_public_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/RoBERTa/data/test_public', forTest=True)

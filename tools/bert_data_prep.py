...
Original file is located at
    https://colab.research.google.com/drive/1Gmz9TVpVTsyDWOQM4RppmIe4xtG9BhXp
"""
...
bert_tokenizer_transformer = BertTokenizer.from_pretrained('bert-base-cased', do_lower_case=False)
...
def encode_examples(df, PATH, sample_weights=None, labels=None, labels_aux=None, forTest=False):
  ...
  for comment in df['comment_text'].astype(str).values:
    bert_input = bert_tokenizer_transformer.encode_plus(comment,
  ...
  with open(PATH + '/input_ids2.npy', 'wb') as filehandle:
    # store the data as binary data stream
    np.save(filehandle,np.asarray(input_ids_list))
  with open(PATH + '/segment_ids2.npy', 'wb') as filehandle:
    # store the data as binary data stream
    np.save(filehandle,np.asarray(token_type_ids_list))
  with open(PATH + '/input_mask2.npy', 'wb') as filehandle:
    # store the data as binary data stream
    np.save(filehandle,np.asarray(attention_mask_list))
  
  if not forTest:
    with open(PATH + '/labels2.npy', 'wb') as filehandle:
      # store the data as binary data stream
      np.save(filehandle , np.asarray(labels))
    with open(PATH + '/labels_aux2.npy', 'wb') as filehandle:
      # store the data as binary data stream
      np.save(filehandle ,np.asarray(labels_aux))
    with open(PATH + '/sample_weights2.npy', 'wb') as filehandle:
      # store the data as binary data stream
      np.save(filehandle,np.asarray(sample_weights))


print("Start preprocess..")
encode_examples(train_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/BERT/data/train', sample_weights_train, y_train, y_train_aux)
print("Finished train data..")
encode_examples(val_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/BERT/data/val', sample_weights_val, y_val, y_val_aux)
print("Finished val data..")
encode_examples(test_private_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/BERT/data/test_private', forTest=True)
print("Finished test private data..")
encode_examples(test_public_df, '/content/drive/My Drive/Jigsaw Unintended Bias in Toxicity Classification/models/BERT/data/test_public', forTest=True)
